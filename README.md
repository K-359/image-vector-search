...
train / val / test は **生成元画像ID単位** で分割します (既定 0.7 / 0.1 / 0.2)。

BDD100Kの100K image taskでは、各動画から抽出された画像を使い、ハイフンを含むstem全体が
画像の識別子です。例えば `0000f77c-6257be58` をハイフンで分割して動画IDとフレームIDに
解釈してはいけません。MOTの連続フレームを別途使う場合は、ファイル名を推測せず、
アノテーションメタデータの `videoName` をグループキーとして使用してください
([BDD100K公式論文](https://arxiv.org/abs/1805.04687))。

クエリは生成元画像の分割に属し、候補画像も同じ分割のプールからのみ選ぶため、
画像もクエリも生成元画像IDも分割をまたぎません。
...
`--ignore-manifest` を付けない限り、マニフェストが存在しない場合や空の場合はエラーで停止します。
...
外部captionを指定した場合は対象画像の100% coverageが必須です。
...
欠損したcaptionと画像のみの候補を意図的に混在させる比較実験では、データ構築・学習・評価の
各コマンドへ `--allow-partial-captions` を明示指定してください。
...
各学習は `--output-dir/runs/<run-id>/` へ独立して保存されます。エポックごとと学習終了時に
valを評価し、nDCG@5がベースラインを超えたrunだけ、そのrunの `best/` を
`--output-dir/best` シンボリックリンクとして昇格します。最終状態は同じrunの `last/` に保存します。
**一度もベースラインを超えなかった場合、そのrunはbestへ昇格しません。**
ベースモデル以下のアダプタを「学習済みモデル」として評価してしまうのを防ぐためです
(この場合 `runs/<run-id>/last/` を明示的に指定すれば評価できます)。以前のbestは削除・変更されず、
入力不備、OOM、中断などで新しい学習が失敗しても保持されます。

gradient accumulationがエポック末で割り切れない場合も、端数の勾配を平均化して
optimizerへ反映してからvalを評価します。
...
base と adapter の差は、**生成元画像単位** のペアード・クラスタ・ブートストラップで
95% 信頼区間と p 値を出します。信頼区間が 0 をまたぐ場合、その差は統計的に有意ではありません。

1枚の画像から最大2件のクエリが作られ、同じ候補集合を共有するため、クエリ単位で復元抽出すると
相関を無視して信頼区間が実際より狭くなります。抽出単位には完全な `source_image_id` を使います。

評価コマンドへモデル出力ディレクトリを指定すると、昇格済みの `best/` を自動的に解決します。
旧形式の直下アダプタも引き続き読み込めます。

...
CAPTION_PROMPT = (
    "入力画像は車内または車載カメラから撮影された交通シーンです。"
    "画像検索に使うため、見える事実にもとづく日本語キャプションを4文程度で出力してください。"
    "道路環境、車両・自転車・歩行者などの対象物、自車から見た位置、進行方向、動きを具体的に書いてください。"
    "危険、ヒヤリハット、接近、飛び出し、逆走、信号無視、車線はみ出し、急停止などに見える要素があれば、その内容と該当する語を明示してください。"
    "見えないことや不確実なことは断定しないでください。"
    "改行や段落は必要ありません。"
)
